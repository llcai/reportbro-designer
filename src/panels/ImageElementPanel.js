...
export default class ImageElementPanel {
    ...
    render() {
        ...
        elDiv = $('<div class="rbroFormRow" id="rbro_image_element_graph_row"></div>');
        elDiv.append(`<label for="rbro_image_element_graph">${this.rb.getLabel('parameterGraph')}:</label>`);
        elFormField = $('<div class="rbroFormField"></div>');
        let elGraph = $('<input id="rbro_image_element_graph" type="checkbox">')
            .change(event => {
                console.log(elGraph.is(":checked"));
                if(elGraph.is(":checked")==true)
                {
                    $('#rbro_image_element_graph_type_row').show();
                }
                else
                {
                    $('#rbro_image_element_graph_type_row').hide();
                    
                    $('#rbro_image_element_source_x_row').hide();
                    $('#rbro_image_element_source_y_row').hide();
                    $('#rbro_image_element_source_y2_row').hide();
                ...
                if (this.rb.getDataObject(this.selectedObjId) !== null) {
                    ...
                    let cmd = new SetValueCmd(this.selectedObjId,
                        'rbro_image_element_graph', 'graph',
                        elGraph.is(":checked"), SetValueCmd.type.checkbox, this.rb);
                    this.rb.executeCommand(cmd);
                }
            });
        elFormField.append(elGraph);
        elDiv.append(elFormField);
        panel.append(elDiv);
        ...
        let elGraphSectionDiv = $('<div id="rbro_image_element_graph_type_row" style="display:none"></div>');
        elDiv = $('<div class="rbroFormRow"></div>');
        elDiv.append(`<label for="rbro_image_element_graph_type">${this.rb.getLabel('parameterGraphType')}:</label>`);
        elFormField = $('<div class="rbroFormField"></div>');
        this.elGraphType = $(`<select id="rbro_image_element_graph_type">
                            <option value="pie">${this.rb.getLabel('chart_pie')}</option>
                            <option value="bar">${this.rb.getLabel('chart_bar')}</option>
                            <option value="barandline">${this.rb.getLabel('chart_barandline')}</option>
                            </select>`)
            .change(event => {
                if(this.elGraphType.val()=='pie')
                {
                    $('#rbro_image_element_source_x_row').show();
                    $('#rbro_image_element_source_y_row').show();
                    $('#rbro_image_element_source_y2_row').hide();
                }
                
                if(this.elGraphType.val()=='bar')
                {
                    $('#rbro_image_element_source_x_row').show();
                    $('#rbro_image_element_source_y_row').show();
                    $('#rbro_image_element_source_y2_row').hide();
                ...
                if(this.elGraphType.val()=='barandline')
                {
                    $('#rbro_image_element_source_x_row').show();
                    $('#rbro_image_element_source_y_row').show();
                    $('#rbro_image_element_source_y2_row').show();
                }
                
                
                if (this.rb.getDataObject(this.selectedObjId) !== null) {
                    let cmd = new SetValueCmd(this.selectedObjId, 'rbro_image_element_graph_type', 'graph_type',
                        this.elGraphType.val(), SetValueCmd.type.select, this.rb);
                    this.rb.executeCommand(cmd);
                }
            });
        elFormField.append(this.elGraphType);
        elDiv.append(elFormField);
        elGraphSectionDiv.append(elDiv);

        //let elGraphTypeDiv = $('<div id="rbro_image_element_graph_type_settings"></div>');
        //panel.renderStyle(elGraphTypeDiv, 'text_element_', '', DocElement.type.text, this, this.rb);
        //elGraphSectionDiv.append(elGraphTypeDiv);
        panel.append(elGraphSectionDiv);


        elDiv = $('<div id="rbro_image_element_source_x_row" class="rbroFormRow"></div>');
        elDiv.append(`<label for="rbro_image_element_source_x">${this.rb.getLabel('imageElementSourceX')}:</label>`);
        elFormField = $('<div class="rbroFormField rbroSplit rbroSelector"></div>');
        let elSourceX = $(`<textarea id="rbro_image_element_source_x" rows="1"></textarea>`)
            .on('input', event => {
                if (this.rb.getDataObject(this.selectedObjId) !== null) {
                    let cmd = new SetValueCmd(this.selectedObjId, 'rbro_image_element_source_x', 'source_x',
                    elSourceX.val(), SetValueCmd.type.text, this.rb);
                    this.rb.executeCommand(cmd);
                }
            });
        autosize(elSourceX);
        elFormField.append(elSourceX);
        elParameterButton = $('<div class="rbroButton rbroRoundButton rbroIcon-select"></div>')
            .click(event => {
                let selectedObj = this.rb.getDataObject(this.selectedObjId);
                if (selectedObj !== null) {
                    this.rb.getPopupWindow().show(this.rb.getParameterItems(selectedObj,
                        [Parameter.type.array]), this.selectedObjId,
                        'rbro_image_element_source_x', 'source_x', PopupWindow.type.parameterSet);
                }
            });
        elFormField.append(elParameterButton);
        elDiv.append(elFormField);
        ...
        elDiv = $('<div id="rbro_image_element_source_y_row" class="rbroFormRow"></div>');
        elDiv.append(`<label for="rbro_image_element_source_y">${this.rb.getLabel('imageElementSourceY')}:</label>`);
        elFormField = $('<div class="rbroFormField rbroSplit rbroSelector"></div>');
        let elSourceY = $(`<textarea id="rbro_image_element_source_y" rows="1"></textarea>`)
            .on('input', event => {
                if (this.rb.getDataObject(this.selectedObjId) !== null) {
                    let cmd = new SetValueCmd(this.selectedObjId, 'rbro_image_element_source_y', 'source_y',
                    elSourceY.val(), SetValueCmd.type.text, this.rb);
                    this.rb.executeCommand(cmd);
                }
            });
        autosize(elSourceY);
        elFormField.append(elSourceY);
        elParameterButton = $('<div class="rbroButton rbroRoundButton rbroIcon-select"></div>')
            .click(event => {
                let selectedObj = this.rb.getDataObject(this.selectedObjId);
                if (selectedObj !== null) {
                    this.rb.getPopupWindow().show(this.rb.getParameterItems(selectedObj,
                        [Parameter.type.number, Parameter.type.string]), this.selectedObjId,
                        'rbro_image_element_source_y', 'source_y', PopupWindow.type.parameterSet);
                }
            });
        elFormField.append(elParameterButton);
        elDiv.append(elFormField);
        ...
        elDiv = $('<div id="rbro_image_element_source_y2_row" class="rbroFormRow"></div>');
        elDiv.append(`<label for="rbro_image_element_source_y2">${this.rb.getLabel('imageElementSourceY2')}:</label>`);
        elFormField = $('<div class="rbroFormField rbroSplit rbroSelector"></div>');
        let elSourceY2 = $(`<textarea id="rbro_image_element_source_y2" rows="1"></textarea>`)
            .on('input', event => {
                if (this.rb.getDataObject(this.selectedObjId) !== null) {
                    let cmd = new SetValueCmd(this.selectedObjId, 'rbro_image_element_source_y2', 'source_y2',
                    elSourceY2.val(), SetValueCmd.type.text, this.rb);
                    this.rb.executeCommand(cmd);
                }
            });
        autosize(elSourceY2);
        elFormField.append(elSourceY2);
        elParameterButton = $('<div class="rbroButton rbroRoundButton rbroIcon-select"></div>')
            .click(event => {
                let selectedObj = this.rb.getDataObject(this.selectedObjId);
                if (selectedObj !== null) {
                    this.rb.getPopupWindow().show(this.rb.getParameterItems(selectedObj,
                        [Parameter.type.number, Parameter.type.string]), this.selectedObjId,
                        'rbro_image_element_source_y2', 'source_y2', PopupWindow.type.parameterSet);
                }
            });
        elFormField.append(elParameterButton);
        elDiv.append(elFormField);
    ...
    updateData(data) {
        if (data !== null) {
            ...
            $('#rbro_image_element_graph').prop('checked', data.getValue('graph'));

            $('#rbro_image_element_graph_type').val(data.getValue('graph_type'));
            $('#rbro_image_element_source_x').val(data.getValue('source_x'));
            $('#rbro_image_element_source_y').val(data.getValue('source_y'));
            $('#rbro_image_element_source_y2').val(data.getValue('source_y2'));

            if(data.getValue('graph'))
            {
                $('#rbro_image_element_graph_type_row').show();
                if(data.getValue('graph_type')=='pie')
                {
                    $('#rbro_image_element_source_x_row').show();
                    $('#rbro_image_element_source_y_row').show();
                    $('#rbro_image_element_source_y2_row').hide();
                ...
                if(data.getValue('graph_type')=='bar')
                {
                    $('#rbro_image_element_source_x_row').show();
                    $('#rbro_image_element_source_y_row').show();
                    $('#rbro_image_element_source_y2_row').hide();
                ...
                if(data.getValue('graph_type')=='barandline')
                {
                    $('#rbro_image_element_source_x_row').show();
                    $('#rbro_image_element_source_y_row').show();
                    $('#rbro_image_element_source_y2_row').show();
